...
<div class="p-4 sm:ml-64" style="margin-top:90px"> 
  <div class="max-w-full mx-auto bg-white shadow-lg rounded-lg">
    ...
    <div class="p-6 flex items-center border-b">
        <img src="{{asset('img/avator.jpg')}}" alt="Profile Picture" class="w-16 h-16 rounded-full border-2 border-gray-300">
        <div class="ml-4">
            <h2 class="text-xl font-semibold">{{auth()->user()->name}}</h2>
            <p class="text-gray-600">Manager</p>
            <p class="text-gray-600">{{auth()->user()->email}}</p>
        </div>
        <button id="editProfileBtn" class="ml-auto text-blue-500 hover:text-blue-700">Edit Profile</button>
    </div>
    ...
    <div class="p-6 border-b">
        ...
        <div class="flex justify-between mb-2">
            <div>
                ...
                <p class="font-medium">{{auth()->user()->phoneNo}}</p>
            </div>
        </div>
    </div>
</div>
   ...
   <!-- Modal -->
   <div id="editProfileModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center hidden">
        ...
        <form action="{{route('updateProfile')}}" method="POST">
          @csrf
            <div class="mb-4">
                ...
                <input name="name" type="text" value="{{auth()->user()->name}}" id="firstName" class="w-full border-gray-300 rounded-lg p-2" placeholder="First Name">
            </div>
            <div class="mb-4">
                <label for="email" class="block text-gray-700">Email</label>
                <input name="email" type="email" value="{{auth()->user()->email}}" id="email" class="w-full border-gray-300 rounded-lg p-2" placeholder="Email">
            </div>
            <div class="mb-4">
                <label for="phone" class="block text-gray-700">Phone</label>
                <input name="phoneNo" type="number" value="{{auth()->user()->phoneNo}}" id="phone" class="w-full border-gray-300 rounded-lg p-2" placeholder="Phone">
            </div>
            <div class="mb-4">
                <label for="password" class="block text-gray-700">Password</label>
                <input name="password" type="password" id="password" class="w-full border-gray-300 rounded-lg p-2" placeholder="Password">
            </div>
            <div class="flex justify-end">
                <button type="button" id="closeModalBtn" class="text-gray-700 mr-4">Cancel</button>
