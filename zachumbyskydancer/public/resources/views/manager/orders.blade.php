...
<div class="p-4 sm:ml-64" style="margin-top:90px"> 
    ...
    <h2 class="text-gray-900 dark:text-white text-lg font-medium mb-4">Orders</h2>
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
      ...
      <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
        ...
        <tbody>
          ...
          @php
              // Assuming $order->item_ids contains the JSON string {"3":1}
              $item_ids_json = $order->item_ids; // Directly using the JSON string
              $encoded_item_ids = urlencode($item_ids_json); // URL encode the JSON string
          @endphp
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
               <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                ...
                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                  <a style="color:blue" href="{{route('paymentDetail',['customer_id' => $order->customer_id])}}">
                    #{{$order->customer_id}}
                  </a>
                </td>
                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                  <a style="color:blue" href="{{route('paymentInvoice',['payment_id' => $order->payment_id,'item_id' => $encoded_item_ids] )}}">
                    #{{$order->payment_id}}
                  </a>
                </td>
                <td class="px-6 py-4 font-semibold" >
                  @if($order->order_status === "Pending")
                    ...
                    @else
                      <span style="color:#0f5061">{{$order->order_status}}</span>
                  @endif
                </td>
                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                    {{$order->order_date}}
                ...
                <td class="px-6 py-4">
                    <a href="{{route('orderInvoice',['order_id' => $order->order_id,'payment_id'=>$order->payment_id])}}" class="inline-block">
